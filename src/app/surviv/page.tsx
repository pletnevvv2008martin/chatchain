...
import { 
  ArrowLeft, Crosshair, Heart, Shield, Target, Package, 
...
interface Achievement {
  ...
  emoji: string;
  description: string;
  condition: (stats: SurvivStats) => boolean;
}

const ACHIEVEMENTS: Achievement[] = [
  { id: 'first_blood', name: 'Первая кровь', emoji: '🩸', description: 'Убить первого врага', condition: s => s.totalKills >= 1 },
  { id: 'killer', name: 'Убийца', emoji: '💀', description: 'Убить 10 врагов', condition: s => s.totalKills >= 10 },
  { id: 'hunter', name: 'Охотник', emoji: '🎯', description: 'Убить 50 врагов', condition: s => s.totalKills >= 50 },
  { id: 'predator', name: 'Хищник', emoji: '🦁', description: 'Убить 100 врагов', condition: s => s.totalKills >= 100 },
  { id: 'winner', name: 'Победитель', emoji: '🏆', description: 'Победить в игре', condition: s => s.wins >= 1 },
  { id: 'champion', name: 'Чемпион', emoji: '👑', description: 'Победить 5 раз', condition: s => s.wins >= 5 },
  { id: 'legend', name: 'Легенда', emoji: '⭐', description: 'Победить 10 раз', condition: s => s.wins >= 10 },
  { id: 'survivor', name: 'Выживший', emoji: '🛡️', description: 'Сыграть 10 игр', condition: s => s.gamesPlayed >= 10 },
  { id: 'veteran', name: 'Ветеран', emoji: '🎖️', description: 'Сыграть 50 игр', condition: s => s.gamesPlayed >= 50 },
  { id: 'rich', name: 'Богач', emoji: '💰', description: 'Заработать 100 HP', condition: s => s.totalHPearned >= 100 },
];
...
function loadStats(): SurvivStats {
  if (typeof window === 'undefined') return { gamesPlayed: 0, wins: 0, totalKills: 0, bestKills: 0, totalHPearned: 0 };
  const stored = localStorage.getItem('chatchain_surviv_stats');
  return stored ? JSON.parse(stored) : { gamesPlayed: 0, wins: 0, totalKills: 0, bestKills: 0, totalHPearned: 0 };
}

function saveStats(stats: SurvivStats) {
  localStorage.setItem('chatchain_surviv_stats', JSON.stringify(stats));
}

function addHP(amount: number) {
  const currentHP = parseInt(localStorage.getItem('chatchain_hp') || '0');
  localStorage.setItem('chatchain_hp', String(currentHP + amount));
}

function getUnlockedAchievements(stats: SurvivStats): Achievement[] {
...
export default function SurvivPage() {
  ...
  const [survivStats, setSurvivStats] = useState<SurvivStats>({ gamesPlayed: 0, wins: 0, totalKills: 0, bestKills: 0, totalHPearned: 0 });
  const [earnedHP, setEarnedHP] = useState(0);
  ...
  const finishGame = useCallback((won: boolean, kills: number, position: number) => {
    const oldStats = loadStats();
    const oldAchievements = getUnlockedAchievements(oldStats);
    
    // Рассчитываем HP
    let hpEarned = kills * HP_REWARDS.kill;
    if (won) hpEarned += HP_REWARDS.win;
    else if (position <= 3) hpEarned += HP_REWARDS.top3;
    else if (position <= 5) hpEarned += HP_REWARDS.top5;
    
    // Обновляем статистику
    const newStats: SurvivStats = {
      gamesPlayed: oldStats.gamesPlayed + 1,
      wins: oldStats.wins + (won ? 1 : 0),
      totalKills: oldStats.totalKills + kills,
      bestKills: Math.max(oldStats.bestKills, kills),
      totalHPearned: oldStats.totalHPearned + hpEarned,
    };
    
    saveStats(newStats);
    addHP(hpEarned);
    setSurvivStats(newStats);
    setEarnedHP(hpEarned);
    
    // Проверяем новые достижения
    const newAchievsList = getUnlockedAchievements(newStats);
    const justUnlocked = newAchievsList.filter(
      a => !oldAchievements.some(oa => oa.id === a.id)
    );
    setNewAchievements(justUnlocked);
    
    // Сохраняем достижения в localStorage
    if (newAchievsList.length > 0) {
      localStorage.setItem('chatchain_surviv_achievements', JSON.stringify(
        newAchievsList.map(a => ({ id: a.id, name: a.name, emoji: a.emoji }))
      ));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      bulletsRef.current = bulletsRef.current.filter(bullet => {
        ...
        if (bullet.ownerId !== player.id) {
          ...
          if (dist < PLAYER_SIZE) {
            ...
            if (player.health <= 0 && !gameFinishedRef.current) {
              player.isAlive = false;
              gameFinishedRef.current = true;
              const position = botsRef.current.filter(b => b.isAlive).length + 1;
              finishGame(false, player.kills, position);
              setGameOver(true);
            }
            return false;
          }
      ...
      if (aliveBots === 0 && player.isAlive && !gameFinishedRef.current) {
        gameFinishedRef.current = true;
        finishGame(true, player.kills, 1);
        setGameWon(true);
        setGameOver(true);
      }
    ...
    return (
      <div className="surviv-page game-over">
        <div className="game-over-content">
          ...
          {/* Заработанные HP */}
          <div className="hp-reward">
            <Heart className="w-6 h-6 text-pink-500" />
            <span>+{earnedHP} HP</span>
          </div>
          ...
          <div className="game-stats">
            <div className="stat-item"><span>🎮 Игр:</span><span>{survivStats.gamesPlayed}</span></div>
            <div className="stat-item"><span>🏆 Побед:</span><span>{survivStats.wins}</span></div>
            <div className="stat-item"><span>💀 Убийств:</span><span>{survivStats.totalKills}</span></div>
            <div className="stat-item"><span>⭐ Лучший:</span><span>{survivStats.bestKills} за игру</span></div>
          </div>

...
export default function FortressPage() {
  ...
  const connectToServer = useCallback(async (retryCount = 0) => {
    ...
    console.log('🔌 Подключение к серверу:', GAME_SERVER, '(попытка', retryCount + 1, ')');
    ...
    try {
      ...
      room.onError((err: any) => {
        console.warn('⚠️ Room error, switching to local mode');
        setIsOnline(false);
        initLocalGame();
      });
      
    } catch (err: any) {
      //Seat reservation expired, timeout, connection refused - переключаемся на локальный режим
      const isExpectedError = err.message?.includes('seat reservation') || 
                              err.message?.includes('timeout') ||
                              err.message?.includes('ECONNREFUSED');
      
      if (isExpectedError) {
        console.log('📡 Сервер недоступен, запуск локального режима');
      } else {
        console.warn('⚠️ Connection issue:', err.message);
      }

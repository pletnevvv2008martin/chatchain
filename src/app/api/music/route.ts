...
if (!globalThis.musicState) {
  globalThis.musicState = {
    ...
    startTime: 0, volume: 100, djUserId: null, lastUpdate: Date.now(), heartEvent: null,
  };
}
...
export async function GET() {
  ...
  if (state.isPlaying && state.currentTrack) {
    currentProgress = state.currentTime + (Date.now() - state.startTime) / 1000;
  }
  let heartEvent = state.heartEvent;
  if (heartEvent && Date.now() - heartEvent.timestamp > 3000) {
    heartEvent = null;
    updateMusicState({ heartEvent: null });
  }
  return NextResponse.json({
    success: true, state: {
      currentTrack: state.currentTrack, queue: state.queue, isPlaying: state.isPlaying,
      currentTime: currentProgress, volume: state.volume, djUserId: state.djUserId,
      lastUpdate: state.lastUpdate, heartEvent,
    },
  });
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'play':
        updateMusicState({ isPlaying: true, startTime: Date.now(), djUserId: userId });
        return NextResponse.json({ success: true, state: getMusicState() });

      case 'pause':
        let pausedTime = state.currentTime;
        if (state.isPlaying) pausedTime = state.currentTime + (Date.now() - state.startTime) / 1000;
        updateMusicState({ isPlaying: false, currentTime: pausedTime, djUserId: userId });
        return NextResponse.json({ success: true, state: getMusicState() });

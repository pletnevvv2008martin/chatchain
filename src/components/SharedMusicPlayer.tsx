...
// Разблокировать аудио при первом клике
function unlockAudio() {
  if (globalAudioUnlocked) return;
  globalAudioUnlocked = true;

  // Создаём пустой звук для разблокировки
  try {
...
export function SharedMusicPlayer({ currentUserId, currentUserName, mode = 'full' }: SharedMusicPlayerProps) {
  ...
  // Разблокировка аудио при первом клике на страницу
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const initPlayer = () => {
      ...
      // Если сервер говорит что играет - включаем autoplay
      const shouldAutoplay = serverState.isPlaying && globalAudioUnlocked;
        ...
        const player = new window.YT.Player(newId, {
          ...
          events: {
            onReady: (event) => {
              ...
              setPlayerReady(true);
              console.log('🎵 YouTube player ready, isPlaying:', serverState.isPlaying);

              // Синхронизируем время
              if (serverState.currentTime > 0) {
                event.target.seekTo(serverState.currentTime, true);
              }

              // Если сервер играет - запускаем
              if (serverState.isPlaying) {
                event.target.playVideo();
                console.log('▶️ Auto-started playback');
              }
            ...
            onStateChange: (event) => {
              console.log('🎬 Player state:', event.data, '(1=playing, 2=paused, 0=ended)');
              if (event.data === 0) sendAction('track_ended');
            },
            onError: (event: any) => {
  ...
  // Синхронизация seek с сервером (каждые 5 секунд)
  useEffect(() => {
    if (!playerReady || !serverState?.currentTrack) return;

    const seekSyncInterval = setInterval(() => {
      if (!playerRef.current || !serverState.isPlaying) return;

      const serverTime = serverState.currentTime + (Date.now() - serverState.lastUpdate) / 1000;
      const localTime = playerRef.current.getCurrentTime();
      const drift = Math.abs(serverTime - localTime);

      // Если расхождение больше 3 секунд - синхронизируем
      if (drift > 3) {
        console.log('🔄 Syncing time, drift:', drift.toFixed(1), 's');
        playerRef.current.seekTo(serverTime, true);
      }
    }, 5000);

    return () => clearInterval(seekSyncInterval);
  }, [playerReady, serverState]);
  ...
  const handlePlayPause = useCallback(() => {
    unlockAudio(); // Разблокируем аудио при клике
    if (!isDj) return;

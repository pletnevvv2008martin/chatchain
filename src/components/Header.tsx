...
import {
  ...
  LogOut, ChevronDown, ChevronUp, X, Crown, Menu
} from 'lucide-react';
...
const ALL_GAMES = [
  ...
  { id: 'dating', name: 'Любовь', icon: Heart, color: '#ec4899', href: '/dating', description: 'Найди свою пару 💕' },
...
export default function Header({
  ...
  // Состояния для мобильной версии и сворачивания
  const [isMobile, setIsMobile] = useState(false);
  const [headerExpanded, setHeaderExpanded] = useState(false); // По умолчанию свёрнута
  ...
  return (
    <>
      <header className={`main-header ${!headerExpanded ? 'header-collapsed' : ''}`}>
        {/* Первая строка: Логотип + Пользователь + Действия */}
        <div className="header-row-1">
          <div className="header-left">
            {/* Кнопка раскрытия/скрытия шапки */}
            <button
              className="header-toggle-btn"
              onClick={() => setHeaderExpanded(!headerExpanded)}
              title={headerExpanded ? 'Свернуть шапку' : 'Развернуть шапку'}
            >
              {headerExpanded ? <ChevronUp size={18} /> : <Menu size={18} />}
            </button>
          ...
          {/* Десктоп версия пользователя - только когда раскрыто */}
          {!isMobile && headerExpanded && (
            <div className="header-actions">
              {/* Имя профиля слева от кнопок */}
              ...
              {onRegisterClick && (
                <button className="register-btn" onClick={onRegisterClick}>
                  <PenLine size={16} />
                  <span className="register-text">{isRegistered ? 'Профиль' : 'Вход'}</span>
                </button>
              )}
          ...
          {/* Скрытый онлайн счётчик когда шапка свёрнута */}
          {!isMobile && !headerExpanded && (
            <div className="header-collapsed-actions">
              <div className="notification-btn mini">
                <Bell size={14} />
                <span className="notification-badge mini">{onlineCount}</span>
              </div>
            </div>
          )}
        </div>

        {/* Вторая строка: Языки + Игры (только если развёрнута) */}
        {headerExpanded && (

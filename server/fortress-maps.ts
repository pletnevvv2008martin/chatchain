...
export const FANTASY_MAPS: MapTemplate[] = [
  {
    id: 'green_valley',
    name: '🌿 Зелёная Долина',
...
export function getMapTemplate(mapId: string): MapTemplate | undefined {
  return FANTASY_MAPS.find(m => m.id === mapId);
}

// Получить случайную карту
export function getRandomMap(): MapTemplate {
  return FANTASY_MAPS[Math.floor(Math.random() * FANTASY_MAPS.length)];
}
